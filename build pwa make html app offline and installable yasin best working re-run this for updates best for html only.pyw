...
def discover_assets(project_dir, generated_icons):
    ...
    if not html_files:
        print("❌ Error: No HTML or PHP files found.")
        return [], []
...
def create_manifest(output_dir, icon_metadata, html_files):
    ...
    try:
        with open(start_file_path, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            if match:
                app_title_from_html = match.group(1).strip()
...
def update_html_files(html_files):
    ...
    for html_path in html_files:
        try:
            ...
            content = re.sub(r'<link\s+[^>]*href=["\']favicon\.ico["\'][^>]*>', '', content, flags=re.IGNORECASE)

            def process_script(match):
                full_tag = match.group(0)
                script_body = match.group(1)
                if "pwa-register" in full_tag:
                    return ""
                if "workbox-window" in script_body or "navigator.serviceWorker" in script_body:
                    if len(script_body.strip()) < 1000:
                        return ""
                    else:
                        cleaned_body = re.sub(
                            r"if\s*\(\s*['\"]serviceWorker['\"]\s+in\s+navigator\s*\)\s*\{[\s\S]*?addEventListener\([\s\S]*?register\([\s\S]*?\}\s*\)\s*;?\s*\}",
                            "",
                            script_body
                        )
                        tag_start = re.match(r'(<script\b[^>]*>)', full_tag, re.IGNORECASE).group(1)
                        return f"{tag_start}{cleaned_body}</script>"
                return full_tag
